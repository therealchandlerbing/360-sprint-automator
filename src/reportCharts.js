...
import { CHART_COLORS, DIMENSION_CONFIG, GAUGE_ZONES, REPORT_COLORS } from './reportStyles.js';
 ...
 * With animation: false, rendering is synchronous
 */
    ...
    const datasets = [
      {
        ...
        pointBorderColor: `#${REPORT_COLORS.white}`,
        pointBorderWidth: 2,
        pointRadius: 5,
      },
    ];
      ...
      plugins: [
        {
          ...
          afterDraw: (chartInstance) => {
            ...
            const { width: areaWidth, height: areaHeight } = chartArea;
            const centerX = areaWidth / 2;
            const centerY = areaHeight;

            // Calculate needle angle (0-100 maps to 180-0 degrees)
            const angle = Math.PI - (safeScore / 100) * Math.PI;
            const needleLength = areaHeight * 0.75;
            ...
            drawCtx.closePath();
            drawCtx.fillStyle = `#${REPORT_COLORS.primary}`;
            drawCtx.fill();

            // Center circle
            drawCtx.beginPath();
            drawCtx.arc(0, 0, 8, 0, Math.PI * 2);
            drawCtx.fillStyle = `#${REPORT_COLORS.primary}`;
            drawCtx.fill();

            // Score text
            drawCtx.font = 'bold 24px Arial';
            drawCtx.fillStyle = `#${REPORT_COLORS.textPrimary}`;
            drawCtx.textAlign = 'center';
            drawCtx.fillText(String(safeScore), 0, -20);
        ...
        datasets: [
          {
            ...
            borderColor: `#${REPORT_COLORS.white}`,
            borderWidth: 2,
          },
        ],
      },
